...
<div class='container'>
  ...
  <div class='row'>
    ...
    <div class='col-4'>
      <h6><b>ชื่อผู้รับสินค้า : {{Auth::user()->name}}</b></h6>
    </div>
    ...
    <table class="table table-boederless table-striped table table-hover">
      ...
      <tbody class='text-center'>
        ...
        <tr class='fs-6'>
          ...
          <th>{{$row->quantity}} </th>
          <th>
            <form method="post" action="{{ route('Importorder.update',$row->id) }}">
              @csrf
              @method('PUT')
  ...
  <div class="card" style="width: 80rem; border-radius: 0px;">
    <div class='row'>
      ...
      <div style="width:240px;height:30px;;">
        <h6 style="text-align:center">{{$order->Total_price}}</h6>
      </div>
    </div>
  </div>
  ...
  <div class='row'>
    ...
    <div class='col-2' style="margin-top:10px;">
        ...
        <a href = "{{route('billImportorderDetail')}}" style = "color:black">พิมพ์</a>
    </div>             
    <div class='col-1'>
    ...
    <div class='col-2'>
      <form method="post" action="{{ route('ImportOrder.storeOrder',$order->id) }}">
        @csrf
